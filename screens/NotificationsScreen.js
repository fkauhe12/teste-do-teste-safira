...
export default function NotificationsScreen({ navigation }) {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(
      ...
      (err) => {
        if (err?.message && /permission|insufficient/i.test(err.message)) {
          console.warn(
            "Sem permissão para ler notifications. Verifique regras do Firestore."
          );
        } else {
  ...
  const openNotification = async (n) => {
    ...
    // SACOLA -> vai para o CARRINHO
    if (n.type === "cart") {
      navigation.navigate("Cart");
      return;
    }

    // PEDIDO -> tela de status da entrega
    if (n.orderId) {
      navigation.navigate("EntregaStatus", { orderId: n.orderId });
      return;
    }

    // PRODUTO ESPECÍFICO -> detalhes do produto
    if (n.productId) {
      navigation.navigate("ProductDetail", { produtoId: n.productId });
      return;
    }
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <Text
            style={{ textAlign: "center", color: "#777", marginTop: 16 }}
          >
